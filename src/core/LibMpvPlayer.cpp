...
bool LibMpvPlayer::start()
{
    ...
    m_thread = std::thread(&LibMpvPlayer::eventLoop, this);

    emit statusChanged(QStringLiteral("libmpv 已就绪"));
    return true;
}

void LibMpvPlayer::shutdown()
{
...
void LibMpvPlayer::load(const QString &uri)
{
...
void LibMpvPlayer::seekTo(double seconds)
{
    if (!m_mpv)
        return;
...
void LibMpvPlayer::setVolumePercent(int percent)
{
    if (!m_mpv)
        return;
...
qint64 LibMpvPlayer::mediaSizeBytes() const
{
    if (!m_mpv)
        return 0;
...
bool LibMpvPlayer::setPictureControl(const QString &name, int value)
{
    if (!m_mpv)
        return false;
...
int LibMpvPlayer::pictureControlValue(const QString &name) const
{
    if (!m_mpv)
        return 0;
...
bool LibMpvPlayer::applySharpen(int value)
{
