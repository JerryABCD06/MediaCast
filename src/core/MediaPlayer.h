 ...
 * 一个"画面调节项"的描述。
 *
 * 界面靠它自己长出一排滑块来 —— 名字、量程、中性值都在这里，界面不需要知道
 * 底下是 mpv 还是别的什么，也不需要为每一项单独写一段代码。
 *
 * **这里没有"给人看的名字"，是有意的。** 早先这个结构体里有一个 label 字段，
 * 中文名字就写死在 C++ 的表里（"亮度""对比度"……）—— 那是要显示给用户看的字，
 * 却绕过了语言文件：界面切到英文，这几行照样蹦中文。现在只给中性的 name，
 * 界面拿 `ui_picture_<name>` 这个键名去语言文件里查。
 */
struct PictureControlInfo
{
    QString name;      // 短名（"brightness"）。命令、查询、查译文都用它，别改。
    int     min = 0;
    int     max = 0;
