...
## 补丁 6：无边框助手认"当前的"原生窗口句柄

**文件**：`FluentUI/src/FluFrameless.cpp`（`nativeEventFilter` 开头）
...
-    if (wid != _current) {
+    if (wid != reinterpret_cast<quint64>(window()->winId())) {
+        _current = wid;
         return false;
     }
```

**为什么**：上游只认**创建窗口那一次**记下的句柄（`_current` 只在初始化里赋值）。
而这个工程的"全屏"切换会主动 `QWindow::destroy()` + `show()` **把原生窗口拆掉
重建**（2026-09-14 修"全屏切回来整窗黑"时定的做法，见 `docs/待办.md`）——
重建之后句柄换了，过滤器就把新窗口的所有消息都放过去：**无边框助手等于失效**，
表现是窗口顶上冒出一条系统标题栏、拖拽/缩放/贴靠也都不管了。

改成跟当前句柄比：这个事件过滤器本来就属于这扇窗，认当前句柄才对（顺手把
`_current` 更新上，保持它和实际一致）。
